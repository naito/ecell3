...
try:
    ...
    class GtkBuilderConverter(object):
        ...
        def _convert(self, klass, node):
            ...
            elif klass == 'GtkToolButton' or klass == 'GtkToggleToolButton':
                self._convert_toolbutton(node)
        ...
        def _convert_toolbutton(self, node):
            icon_node = None            
            for child in node.childNodes:
                if child.nodeType == Node.ELEMENT_NODE and \
                   child.tagName == "property":
                    if child.getAttribute("name") == "icon":
                        icon_node = child
            if icon_node is not None:
                filename = icon_node.firstChild.nodeValue
                obj_id = icon_node.getAttribute("id") + str(hash(icon_node))
                obj= self._dom.createElement("object")
                obj.setAttribute("class", "GtkImage")
                obj.setAttribute("id", obj_id)
                prop = self._dom.createElement("property")
                prop.setAttribute("name", "visible")
                prop.appendChild(self._dom.createTextNode("True"))
                obj.appendChild(prop)
                prop = self._dom.createElement("property")
                prop.setAttribute("name", "pixbuf")
                prop.appendChild(self._dom.createTextNode(filename))
                obj.appendChild(prop)
                icon_node.setAttribute("name", "icon_widget")
                icon_node.firstChild.nodeValue = obj_id
                self._dom.documentElement.appendChild(obj)
